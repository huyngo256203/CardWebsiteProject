...
public class CardController {
    ...
    public String displayCard(Model model, @RequestParam(defaultValue = "0") Integer pageNo) {
        ...
        Page<Card> pageList = cardService.findAllCards(pageNo, 4);
        if (pageList.isEmpty()) {
    ...
    @PostMapping("/create")
    public String createCard(Model model, @Valid @ModelAttribute("cardDTO") CardDTO cardDTO,
        ...
        for (Card card : listCard) {
            ...
            if (card.getCardNumber().equals(cardDTO.getCardNumber())) {
                if (card.getCardType().getPublisher().getId() == cardDTO.getCardType().getPublisher().getId()) {
                    result.rejectValue("cardNumber", "cardNumber.error", "Card number is already exist");
                    return "/cardAdmin/create";
                }
            }
        }
    ...
    public void exportCardsToExcel(HttpServletResponse response) {
        ...
        try {
            Workbook workbook = service.exportCardsToExcel();
